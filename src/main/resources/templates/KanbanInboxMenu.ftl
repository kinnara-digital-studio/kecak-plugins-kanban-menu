...
<body>
	...
	<script>
		...
		var dragTargetMap = JSON.parse('${dragTargets!"{}"}');
        ...
        function moveCard(cardId, targetBoardId, sourceBoardId, el) {
            ...
            var targetsFromHere = dragTargetMap[sourceBoardId] || {};
            var targetInfo = targetsFromHere[targetBoardId];

            if (!targetInfo) {
                alert("This move is not allowed by the process flow");
                revertCard(cardId, el, sourceBoardId);
                return;
            }

            var body = {};
            (targetInfo.conditions || []).forEach(function(cond) {
                body[cond.variable] = cond.value;
            });
            ...
            jQuery.ajax({
                ...
                success: function(resp) {
                    ...
                    if (isError) {
                        ...
                        if (resp.validation_error) {
                            var fieldMessages = [];
                            for (var fieldId in resp.validation_error) {
                                if (resp.validation_error.hasOwnProperty(fieldId)) {
                                    fieldMessages.push(fieldId + ": " + resp.validation_error[fieldId]);
                                }
                            }
                            errors = fieldMessages.join("\n");
                        } else if (resp.errors && typeof resp.errors === 'object') {
                            var fieldMessages2 = [];
                            for (var fieldId2 in resp.errors) {
                                if (resp.errors.hasOwnProperty(fieldId2)) {
                                    fieldMessages2.push(fieldId2 + ": " + resp.errors[fieldId2]);
                                }
                            }
                            errors = fieldMessages2.join("\n");
                ...
                error: function(xhr) {
                    var errorDetail = xhr.responseText || ("HTTP " + xhr.status);
                    try {
                        var parsed = JSON.parse(xhr.responseText);
                        if (parsed.validation_error) {
                            var msgs = [];
                            for (var fid in parsed.validation_error) {
                                if (parsed.validation_error.hasOwnProperty(fid)) {
                                    msgs.push(fid + ": " + parsed.validation_error[fid]);
                                }
                            }
                            errorDetail = msgs.join("\n");
